import pytest
from click.testing import CliRunner

from web_crawler.cli import crawl
from web_crawler.crawler import DEFAULT_CHECK_HEAD
from web_crawler.crawler import DEFAULT_MAX_WORKERS
from web_crawler.crawler import DEFAULT_OBEY_ROBOTS
from web_crawler.crawler import DEFAULT_TIMEOUT
from web_crawler.crawler import DEFAULT_USER_AGENT

TEST_URL = "https://www.example.com"


@pytest.mark.parametrize("user_agent", [("-u", "Tester"), ("--user-agent", "Tester"), None])
@pytest.mark.parametrize("max_workers", [("-w", "10"), ("--max-workers", "10"), None])
@pytest.mark.parametrize("timeout", [("-t", "30"), ("--timeout", "30"), None])
@pytest.mark.parametrize("check_head", ["-h", "--check-head", None])
@pytest.mark.parametrize("obey_robots", ["-o", "--obey-robots", None])
def test_crawl(user_agent, max_workers, timeout, check_head, obey_robots):
    runner = CliRunner()

    user_agent_arg = [] if user_agent is None else [user_agent[0], user_agent[1]]
    max_workers_arg = [] if max_workers is None else [max_workers[0], max_workers[1]]
    timeout_arg = [] if timeout is None else [timeout[0], timeout[1]]
    check_head_arg = [] if check_head is None else [check_head]
    obey_robots_arg = [] if obey_robots is None else [obey_robots]

    result = runner.invoke(
        crawl,
        [TEST_URL]
        + user_agent_arg
        + max_workers_arg
        + timeout_arg
        + check_head_arg
        + obey_robots_arg
        + ["--debug"],
    )
    assert result.exit_code == 0

    assert (
        result.output == f"Crawling URL: {TEST_URL}\n"
        f"Debug mode is on: crawling not running\n"
        f"crawler User-Agent: {user_agent[1] if user_agent else DEFAULT_USER_AGENT}\n"
        f"crawler max workers: {max_workers[1] if max_workers else DEFAULT_MAX_WORKERS}\n"
        f"crawler timeout: {timeout[1] if timeout else DEFAULT_TIMEOUT}\n"
        f"crawler check head: {not DEFAULT_CHECK_HEAD if check_head else DEFAULT_CHECK_HEAD}\n"
        f"crawler obey robots: {not DEFAULT_OBEY_ROBOTS if obey_robots else DEFAULT_OBEY_ROBOTS}\n"
    )
